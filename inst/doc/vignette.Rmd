...
*hopit* is an open source software library written in R [@Rteam2019] and C++ [@RcppEigen2013; @Rcpp2011] languages. The *hopit* package provides versatile methods to fit and analyze ordered response data in the context of self reporting styles.
...
The thresholds (cut points, $\alpha$) are modeled by threshold variables $Y$, their coefficents $\gamma$, and intercepts $\lambda$. It is assumed that they model contextual characteristics of the respondent (e.g. country, gender, age, etc. ). Threshold variables are modeled without parallel regression assumption, thus each threshold is modeled by a variable independently [@Boes2006; @Green2014]. 
