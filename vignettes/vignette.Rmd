...
*model1* contains 11 dichotomous health variables and two threshold variables. It can be further extended by adding country of origin to the treshold formula to better control for cultural differences.
...
Including survey design could be important as it can affect the results, in presented example has however an minor importance, which is seen by comparing coefficients of latent variable for both models:
...
The fit of the model can be assessed using *profile*() function, which calculate and plot profile of the log likelihood function around fitted coefficient values.
