...
```{r, echo=TRUE, cache=TRUE}
model4 <- hopit(reg.formula = health ~ hypertenssion + cholesterol + heart_stroke_respiratory +
                 poor_mobility_or_grip + depression ,
               thresh.formula = ~ sex + ageclass + country,
               control=list(trace=FALSE),
               overdispersion = TRUE,
...
unname(exp(model4$coef.ls$logTheta)) #Theta
...
```{r, echo=TRUE}
standardizeCoef(model2, plotf = TRUE, namesf = function(x) substr(x,1,nchar(x)-3))
