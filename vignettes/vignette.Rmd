...
abstract: The *gotm* package provides R functions to fit and analyze ordered response data in the context of reporting styles. In this vignette we describe the formulation and fit of *gotm* models as well as functions used to analyse reporting styles.
date: "14 XII 2018"
output: pdf_document
---
...
The *gotm* package provides R functions to fit and analyze ordered response data in the context of reporting styles.
...
.......how the fitting is performed in the package vglm bfgs/cg
...
```{r, echo=TRUE}
model1<- gotm(reg.formula = health ~ hypertenssion + cholesterol + heart_stroke_respiratory + 
...
model2<- gotm(reg.formula = health ~ hypertenssion + cholesterol + heart_stroke_respiratory + 
                 poor_mobility_or_grip + depression ,
...
```{r, echo=TRUE}
model3<- gotm(reg.formula = health ~ hypertenssion + cholesterol + heart_stroke_respiratory + 
                 poor_mobility_or_grip + depression ,
               thresh.formula = ~ sex * ageclass + country,
               control=list(trace=FALSE),
               data = healthsurvey)
...
The *gotm*() function has also an option to include survey design using *survey* package. 
...
model2s<- gotm(reg.formula = health ~ hypertenssion + cholesterol + heart_stroke_respiratory + 
                 poor_mobility_or_grip + depression ,
               thresh.formula = ~ sex + ageclass + country,
               design = design,
               control=list(trace=FALSE),
