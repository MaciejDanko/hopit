...
The Jürges' percentile method is based on on original distribution of categorical response variable. First for each category $j$ an empirical distribution function is constructed.
...
Generalized ordered probit model can be fitted using *hopit*() function. The function takes two kinds of formulas: (1) *reg.formula* that models the latent variable and (2) *thresh.formula that models thresholds.   
...
*model1* can be further extended by adding country of origin to the treshold formula to control for cultural differences.
...
Both *reg.formula* and *thresh.formula* allow to specify interactions, like interaction between gender (*sex*) and age (*ageclass*): 
...
Let's look at latent health variables of *model2*. 

```{r, echo=TRUE}
model3$coef.ls$reg.params
```

We can standardize them using Jürges' approach [@Jurges2007] to obtain disability weights. The standardization can be doen using *standardizeCoef*() function.
...
The *namesf* argument is a function or a character vector that is used to rename the coefficent names. Here, it removes last 3 letters ("yes"), which is a reference level for each variable and exchanges "_" with spaces in variable names.
...
The central part of reporting styles analyses is to determine the cut-points used to calcualte adjusted health status for each individual. The calcualtion and ploting of cut-points is realized by *getCutPoints*() function.
...
The differences between original and adjusted frequncies can be calcualted directly using *getLevels* output:
...
## 7. Bootstraping Confidence Intervals

The package offers functions to calculate confidence intervals for any measure derived from the model. As an example we show calcualtion of confidence intervals  of the difference between original and adjusted frequencies of combined "Poor" + "Fair" healt categories.
